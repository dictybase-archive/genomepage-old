...
die "Usage: memcached-tool <host[:port]> [mode]\n
...
if ( $mode eq "move" ) {
    ...
    while (1) {
        ...
        if ( $res eq "DONE" ) {
            ...
            print
                "Error: can't move from $from to $to.  Destination not yet full?  See usage docs.\n";
            exit;
        }
        elsif ( $res eq "BUSY" ) {
            if ( ++$tries == 3 ) {
...
if ( $mode eq 'dump' ) {
    ...
    foreach my $bucket ( sort( keys(%items) ) ) {
        print STDERR "Dumping bucket $bucket - "
            . $items{$bucket}
            . " total items\n";
