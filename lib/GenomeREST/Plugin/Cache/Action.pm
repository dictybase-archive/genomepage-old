package GenomeREST::Plugin::Cache::Action;
...
sub register {
    ...
    $app->log->debug('using driver class '.$cache->driver_class);
    ...
    $app->plugins->add_hook(
        'before_dispatch' => sub {
            ...
            if ( $cache->is_valid($path) ) {
            	...
            	$app->log->debug("no cache found for $path");
            }
        }
    );
